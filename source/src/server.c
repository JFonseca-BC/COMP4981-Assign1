...
void log_message(const char *level, const char *format, ...) {
  ...
  const struct tm *const t =
      localtime_r(&now, &t_buf); /* Fixes cppcheck portability */

  if (t != NULL) {
    char time_str[TIME_STR_SIZE]; /* Reduced scope to fix cppcheck style */
...
/* --- Helper: Serve File via POSIX APIs --- */
void serve_file(int client_fd, const char *path, int is_head) {
  ...
  FILE *const file = fopen(full_path, "rbe");
  if (file == NULL) {
  ...
  const long fsize_raw = ftell(file);
  const size_t fsize = (fsize_raw > 0) ? (size_t)fsize_raw : 0;
